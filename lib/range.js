...
var Range = Element.extend(vertical).extend({
  ...
  snap: function(val) {
    if(this.options.snap) {
      // eww. doing *1/snap is less likely to get us inexact values, as snap ∈ [0,1]
      return Math.round(val * (1/this.options.snap))  * this.options.snap;
    }
    return val;
  },
  ...
  drag: function(origEv) {
    ...
    return function(ev) {
      ...
      if(mousePos) {
        var start = self.snap((mousePos - perantStart - mouseOffset) / perantSize);

        if (start >= 0 && start <= (perantSize - beginSize) / perantSize) {
          var rangeOffset = start - self.range[0];
          self.val([start, self.range[1] + rangeOffset]);
        } else {
